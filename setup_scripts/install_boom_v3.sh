...
echo "Ready to continue with Boom V3 Installation..."
...
PARAMS_FILE="$CHIPYARD_DIR/generators/boom/src/main/scala/v3/common/parameters.scala"
...
CORE_FILE="$CHIPYARD_DIR/generators/boom/src/main/scala/v3/exu/core.scala"

# Path to replacement file from fuzzer setup
NEW_CORE_FILE="$THEHUZZ_ROOT/setup_scripts/boom_v3_setup_files/core.scala"
...
# Path to replacement file from fuzzer setup
NEW_CSR_FILE="$THEHUZZ_ROOT/setup_scripts/boom_v3_setup_files/CSR.scala"
...
NEW_SIMS_DIR="$THEHUZZ_ROOT/setup_scripts/boom_v3_setup_files/sims"
...
###############################################
# Build SmallBoomV3Config if not already built
###############################################

SIMS_VCS_DIR="$CHIPYARD_DIR/sims/vcs"
SIMV_BINARY="$SIMS_VCS_DIR/simv-chipyard.harness-SmallBoomV3Config"
GEN_SRC_DIR="$SIMS_VCS_DIR/generated-src"

echo "Checking if SmallBoomV3Config simulation is already built ..."

if [[ -x "$SIMV_BINARY" && -d "$GEN_SRC_DIR" ]]; then
    echo "Simulation already built: $SIMV_BINARY and $GEN_SRC_DIR exist."
else
    echo "Building SmallBoomV3Config simulation ..."
    cd "$SIMS_VCS_DIR"
    make clean
    make CONFIG=SmallBoomV3Config CHIPYARD_ROOT=$CHIPYARD_DIR
    echo "Build completed."
fi


###############################################
# Copy dramsim2_ini into boomv3_1130
###############################################

SRC_DRAMSIM_DIR="$CHIPYARD_DIR/generators/testchipip/src/main/resources/dramsim2_ini"
DEST_DIR="$THEHUZZ_ROOT/benchmarks/boomv3_1130"
