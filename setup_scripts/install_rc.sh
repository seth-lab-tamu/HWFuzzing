...
echo "Ready to continue with RocketConfig Installation..."
...
NEW_CSR_FILE="$THEHUZZ_ROOT/setup_scripts/rc_setup_files/CSR.scala"
...
NEW_SIMS_DIR="$THEHUZZ_ROOT/setup_scripts/rc_setup_files/sims"
...
###############################################
# Build RocketConfig if not already built
###############################################

SIMS_VCS_DIR="$CHIPYARD_DIR/sims/vcs"
SIMV_BINARY="$SIMS_VCS_DIR/simv-chipyard.harness-RocketConfig"
GEN_SRC_DIR="$SIMS_VCS_DIR/generated-src"

echo "Checking if RocketConfig simulation is already built ..."

if [[ -x "$SIMV_BINARY" && -d "$GEN_SRC_DIR" ]]; then
    echo "Simulation already built: $SIMV_BINARY and $GEN_SRC_DIR exist."
else
    echo "Building RocketConfig simulation ..."
    cd "$SIMS_VCS_DIR"
    make clean
    make CONFIG=RocketConfig CHIPYARD_ROOT=$CHIPYARD_DIR
    echo "Build completed."
fi


###############################################
# Copy dramsim2_ini into rc
###############################################

SRC_DRAMSIM_DIR="$CHIPYARD_DIR/generators/testchipip/src/main/resources/dramsim2_ini"
DEST_DIR="$THEHUZZ_ROOT/benchmarks/rc_1130"
