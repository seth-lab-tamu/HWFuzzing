...
class MStatus extends Bundle {
  ...
  // Starting from here is the content of RV64 mstatus
  val sd = Bool()
  val zero2 = UInt(23.W)
  val mpv = Bool()
  val gva = Bool()
...
class CSRFileIO(hasBeu: Boolean)(implicit p: Parameters) extends CoreBundle
  ...
  // Add for dumping csr information
  val fflags = Output(UInt(5.W))
  val frm = Output(UInt(3.W))
  val fcsr = Output(UInt(8.W))

  // sstatus is the same as mstatus in RC and Boom 
  // sie same as mie 
  val stvec = Output(UInt(xLen.W))
  val scounteren = Output(UInt(32.W))
  val sscratch = Output(UInt(xLen.W))
  val sepc = Output(UInt(vaddrBitsExtended.W))
  val scause = Output(UInt(xLen.W))
  val stval = Output(UInt(xLen.W))
  // sip same as mip
  val satp = Output(new PTBR)

  // mstatus defiend as status 
  // misa defined inside status
  ...
  val mepc = Output(UInt(vaddrBitsExtended.W))
  val mcause = Output(UInt(xLen.W))
  val mtval = Output(UInt(vaddrBitsExtended.W)) //todo, check this, not sure
  val mip = Output(new MIP)
  val minstret = Output(UInt(xLen.W))
 
  // todo
  //val pmpregs = Output(Vec(CSR.maxPMPs, new PMP))
}
...
class CSRFile(
  ...
  io.mie := reg_mie // added for log csr information
  ...
  io.fcsr := Cat(reg_frm, reg_fflags)
  ...
  //printf("reg_pmp value %d\n", reg_pmp.nonEmpty.B.asUInt) // todo dump pmp
  if (reg_pmp.nonEmpty) {
